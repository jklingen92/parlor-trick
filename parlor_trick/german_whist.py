...
from parlor_trick.core import Game, Player
...
class Round(Game):
    ...
    def basic_turn(self, leader_wins, follower_wins):
        ...
        self.message(f"{self.lead} vs {follow}")
    ...
    def foreplay_turn(self):
        ...
        def leader_wins():
            ...
            self.follower.hand.add(self.deck.draw())
            self.message(f"{self.leader.name} wins the {self.prize}")

        def follower_wins():
            self.follower.hand.add(self.deck.draw())
            self.leader.opposing_hand.add(self.prize)
            self.leader.hand.add(self.deck.draw())
            self.message(f"{self.follower.name} wins the {self.prize}")
    ...
    def scoring_turn(self):
        ...
        def leader_wins():
            self.leader.score += 1
            self.message(f"{self.leader.name} wins the trick")

        def follower_wins():
            self.follower.score += 1
            self.message(f"{self.follower.name} wins the trick")
            winner = self.follower
            self.follower = self.leader
            self.leader = winner
...
class GermanWhist(Game):
    ...
    def turn(self):
        ...
        self.score_game(winner, round_score)
